...
for (i in 1:nsim){Y[i,1]=rnorm(1,-b/a,sqrt(sigma**2/2/(-a)));
for (j in 2:N){Y[i,j] = Y[i,j-1] +b*dt+a*Y[i,j-1]*dt +sigma*X[i,j];}}
H<-hist(Y[,N],plot=FALSE); dev.new(width=16,height=7);
